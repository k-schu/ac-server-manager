...
from ac_server_manager.config import AC_SERVER_TCP_PORT
...
def test_status_command_displays_acstuff_url(runner: CliRunner) -> None:
    ...
    with patch("ac_server_manager.cli.Deployer") as MockDeployer:
        ...
        # Check that the new acstuff.ru URL format is displayed
        expected_url = f"https://acstuff.ru/s/q:race/online/join?ip=1.2.3.4:{AC_SERVER_TCP_PORT}"
        assert expected_url in result.output
        assert "acstuff.ru link:" in result.output
        # Ensure old format is not present
        assert "q:race@" not in result.output
